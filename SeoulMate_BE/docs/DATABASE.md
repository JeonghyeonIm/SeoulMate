...
- **DB 엔진**: PostgreSQL 18.3 (AWS RDS, arm64/Graviton)
- **접속**: EC2 → SSH 터널 → RDS (현재 로컬 개발 시 `127.0.0.1:15432`)
...
| 인덱스                         | 컬럼                   | 조건/용도                    |
| ------------------------------ | ---------------------- | ---------------------------- |
| `idx_users_email`              | `email`                | 로그인/계정 조회             |
| `idx_users_preferred_region`   | `preferred_region`     | 선호 지역 기반 조회          |
| `idx_users_preferred_category` | `preferred_category`   | 선호 카테고리 기반 조회      |
...
서울 공개데이터 API/파일 소스를 정규화해 하나의 테이블에 저장하는 통합 저장소.
원본 카테고리(`category`)와 별도로 서비스 추천에 쓰는 내부 장소 분류(`place_*`)와 Kakao Local API 매칭 결과(`kakao_*`)를 함께 보관한다.
...
-- 내부 장소 분류 정규화
place_family        varchar(50)    -- eat | see | do 등 상위 장소 계열
place_type          varchar(100)   -- restaurant, cafe, park, exhibition 등 추천 필터용 타입
place_subtype       varchar(100)   -- korean_restaurant, bakery_cafe 등 세부 타입
category_confidence numeric(4,2)   -- 내부 분류 신뢰도 (0.00 ~ 1.00)

-- Kakao Local API 매칭 보강
kakao_place_name           varchar(255)  -- 카카오 장소명
kakao_place_url            text          -- 카카오 장소 상세 URL
kakao_category_name        varchar(255)  -- 카카오 전체 카테고리 경로
kakao_category_group_name  varchar(100)  -- 카카오 카테고리 그룹명
kakao_match_confidence     numeric(5,2)  -- 자체 매칭 점수
kakao_matched_at           timestamp     -- matched 처리 시각
kakao_checked_at           timestamp     -- matched/skipped 여부를 확인한 시각
kakao_match_status         varchar(20)   -- matched | skipped | NULL(pending)
...
| 인덱스                                      | 컬럼/표현식                        | 용도                         |
| ------------------------------------------- | ---------------------------------- | ---------------------------- |
| `idx_public_data_source_dataset`            | `source_dataset`                   | 데이터셋별 배치/조회         |
| `idx_public_data_category`                  | `category`                         | 원본 카테고리 필터링         |
| `idx_public_data_region`                    | `region`                           | 지역 필터링                  |
| `idx_public_data_title_tsv`                 | `to_tsvector('simple', title)` GIN | 제목 전문검색                |
| `idx_public_data_title_trgm`                | `title gin_trgm_ops`               | 제목 부분/유사 검색          |
| `idx_public_data_category_trgm`             | `category gin_trgm_ops`            | 카테고리 부분/유사 검색      |
| `idx_public_data_region_trgm`               | `region gin_trgm_ops`              | 지역 부분/유사 검색          |
| `idx_public_data_address_trgm`              | `address gin_trgm_ops`             | 주소 부분/유사 검색          |
| `idx_public_data_metadata_text_trgm`        | `(metadata::text) gin_trgm_ops`    | metadata 텍스트 검색         |
| `idx_public_data_place_family`              | `place_family`                     | 내부 장소 계열 필터링        |
| `idx_public_data_place_type`                | `place_type`                       | 내부 장소 타입 필터링        |
| `idx_public_data_place_subtype`             | `place_subtype`                    | 내부 세부 타입 필터링        |
| `idx_public_data_kakao_category_group_name` | `kakao_category_group_name`        | 카카오 그룹 카테고리 분석    |
| `idx_public_data_kakao_category_name`       | `kakao_category_name`              | 카카오 카테고리 분석         |
| `idx_public_data_kakao_checked_at`          | `kakao_checked_at`                 | 카카오 매칭 배치 진행률 조회 |
| `idx_public_data_kakao_match_status`        | `kakao_match_status`               | matched/skipped/pending 조회 |

**트리거**: `updated_at` 자동 갱신 (`set_current_timestamp_updated_at`)

**정규화/매칭 상태**

- `place_family`, `place_type`, `place_subtype`, `category_confidence`: `npm run normalize:public-data-categories` 또는 적재 시 기본 규칙으로 채움.
- `kakao_*`: `npm run normalize:kakao-categories`가 Kakao Local API 검색 결과를 매칭해 채움.
- `kakao_match_status = 'matched'`: 카카오 장소와 충분한 신뢰도로 연결되어 `kakao_*` 정보 및 내부 분류를 보강한 상태.
- `kakao_match_status = 'skipped'`: 카카오 검색 결과가 없거나, 거리/문자열/카테고리 조건을 통과하지 못한 상태.
- `kakao_match_status IS NULL`: 아직 카카오 매칭 job이 확인하지 않은 pending 상태.
...
**인덱스**: `idx_recommendation_requests_user_created_at (user_id, created_at DESC)`
...
| 인덱스                             | 컬럼                           | 용도                    |
| ---------------------------------- | ------------------------------ | ----------------------- |
| `idx_recommendations_public_data`  | `public_data_id`               | 장소별 추천 이력 조회   |
| `idx_recommendations_user_request` | `(user_id, request_id)`        | 사용자별 추천 결과 조회 |
| `uq_recommendations_request_place` | `(request_id, public_data_id)` | 요청 내 장소 중복 방지  |
...
**인덱스**: `idx_saved_courses_user_saved_at (user_id, saved_at DESC)`
...
| -------------------------- | -------------------------- | --------------------- | ------------------- |
| `npm run sync:public-data` | `initial_public_data_sync` | 전체 공개데이터셋     | 초기 1회 또는 수동  |
...
| ----------------------- | ----------------------------- | ------------- | ---------------------------------- | --------- | ------: | ----------- | ---------------- |
| `TbVwRestaurants`       | 서울 관광 음식                | OA-21054      | 서울관광재단 / 스마트관광팀        | 문화/관광 |   1,247 | ❌ (주소만) | upsert           |
...
| `culturalSpaceInfo`     | 서울 문화공간                 | OA-15487      | 서울시 / 문화본부 문화정책과       | 문화/관광 |   1,052 | ✅ WGS84    | upsert (일 1회)  |
| `culturalEventInfo`     | 서울 문화행사                 | OA-15486      | 서울시 / 문화본부 문화정책과       | 문화/관광 |   3,892 | ✅ WGS84    | replace (일 1회) |
| `LOCALDATA_072405`      | 서울시 휴게음식점 인허가 정보 | OA-16095      | 서울시 / 디지털도시국 데이터전략과 | 보건      |  29,672 | ✅ 변환됨   | replace          |
| `LOCALDATA_072404`      | 서울시 일반음식점 인허가 정보 | OA-16094      | 서울시 / 시민건강국 식품정책과     | 보건      | 120,440 | ✅ 변환됨   | replace          |
| `OA-13663`              | 서울시 식품위생업소 현황      | OA-13663      | 서울시 / 디지털도시국 데이터전략과 | 보건      |  미적재 | ❌ (주소만) | replace          |
...
```bash
npm run normalize:public-data-categories
```

- `public_data`의 `category`, `title`, `metadata.businessType` 등을 기반으로 `place_family`, `place_type`, `place_subtype`, `category_confidence`를 채움
- 추천 후보 조회는 원본 `category`뿐 아니라 이 정규화 컬럼을 함께 사용

### Kakao Local API 기반 장소 매칭

```bash
npm run normalize:kakao-categories
```

- 대상: 기본 `LOCALDATA_072404`, `LOCALDATA_072405`
- `kakao_checked_at IS NULL` 행을 배치로 조회해 카카오 키워드 검색을 수행
- 매칭 성공 시 `kakao_place_name`, `kakao_place_url`, `kakao_category_name`, `kakao_category_group_name`, `kakao_match_confidence`, `kakao_matched_at`, `kakao_checked_at`, `kakao_match_status = 'matched'` 저장
- 매칭 실패 시 `kakao_match_status = 'skipped'`, `kakao_checked_at` 저장
- 주요 환경변수:
  - `KAKAO_NORMALIZE_LIMIT`: 배치당 처리 건수
  - `KAKAO_NORMALIZE_DATASETS`: 처리할 source_dataset CSV
  - `KAKAO_NORMALIZE_PLACE_TYPES`: `KAKAO_NORMALIZE_PROCESS_ALL=false`일 때 대상 place_type CSV
  - `KAKAO_NORMALIZE_PROCESS_ALL`: `true`면 place_type 필터 없이 처리
  - `KAKAO_NORMALIZE_REPEAT`: `true`면 남은 행이 없을 때까지 반복
  - `KAKAO_NORMALIZE_MAX_BATCHES`: 반복 처리 최대 배치 수 (`0`이면 제한 없음)
...
| `20260511_add_leisure_permit_datasets.sql`             | 놀거리 인허가 데이터셋 source_dataset 목록/운영 메모 추가                                                |
| `20260511_add_public_data_category_normalization.sql`  | `public_data`에 `place_family`, `place_type`, `place_subtype`, `category_confidence` 추가 및 인덱스 생성 |
| `20260511_add_public_data_kakao_category_columns.sql`  | `public_data`에 Kakao Local API 매칭 컬럼(`kakao_*`) 추가 및 인덱스 생성                                 |
| `20260511_add_public_data_trgm_indexes.sql`            | `pg_trgm` 확장 및 `public_data` 텍스트 검색용 trigram 인덱스 추가                                        |
| `20260511_nullify_invalid_public_data_coordinates.sql` | 서울 범위를 벗어난 비정상 좌표를 null 처리                                                               |
